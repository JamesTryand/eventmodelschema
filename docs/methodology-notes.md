...
Reference material from `resources/event-modeling-cheat-sheet.pdf`, kept here for
context since it informed the schema design but doesn't map to any schema field
directly — these describe how to *run a workshop*, not the shape of a *document*.
Vendor tooling/branding (app.eventmodelers.ai, `@eventmodelers/*` npx kits, book/
course/podcast links) is intentionally omitted — not this project's concern.
...
7. State Change • State View • Automation • Translation.
